...
struct FILTER_DATA {
	BYTE FilterEN;							//滤器使能位，0-禁用；1-使能
	BYTE FilterMode;						//过滤器工作模式，0x00-16位掩码模式；0x01-32位掩码模式；0x10-16位列表模式；0x11-32位列表模式：
	UINT FilterId;							//过滤器ID寄存器
	UINT FilterMask;						//过滤器掩码寄存器
};

typedef struct _INIT_CONFIG {
	struct FILTER_DATA filter_info[14];		//CAN过滤器组
	UINT filter_num; 						//使用过滤器组数量，取值0-13
    DWORD   Reserved;						//保留
    UCHAR   Filter;							//保留
    UCHAR   Timing0;						//保留
    UCHAR   Timing1;						//保留
    UCHAR   Mode;							//CAN工作模式，0：正常；1：只听
    UINT   BotRate;							//CAN通讯波特率
